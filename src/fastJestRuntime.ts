/**
 * This is basically just a copy of the normal jest 29.7.0 runtime with a few snippets modified
 * P.S. Facebook sucks and the metaverse is hella dumb lol.
 */
/**
...
export default class FastJestRuntime {
  ...
  private readonly ignoreFastCache: boolean = false;
  ...
  constructor(
    ...
    // @ts-ignore
    this.ignoreFastCache = environment.ignoreFastCache;
  ...
  private async loadEsmModule(
    ...
    if (!registry.has(cacheKey)) {
      ...
      if (isWasm(modulePath)) {
        const wasm: any = this._importWasmModule(
